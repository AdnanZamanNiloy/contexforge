...
class Document:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.text, str) or not self.text.strip():
            raise ValueError(f"Document '{self.document_id}': text must be a non-blank string")
...
class Chunk:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.text, str) or not self.text.strip():
            raise ValueError(f"Chunk '{self.chunk_id}': text must be a non-blank string")
        if isinstance(self.metadata, dict):
            object.__setattr__(self, "metadata", _freeze_metadata(self.metadata))
...
class RetrievedChunk:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.chunk, Chunk):
            raise TypeError(f"RetrievedChunk.chunk must be a Chunk, got {type(self.chunk).__name__}")
...
class RerankedChunk:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.chunk, Chunk):
            raise TypeError(f"RerankedChunk.chunk must be a Chunk, got {type(self.chunk).__name__}")
