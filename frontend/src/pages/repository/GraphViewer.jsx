    ...
    if (dims && selected) {
      ...
      if (n) {
        ...
        applyView(
          {
            ...view,
            x: clientWidth / 2 - n.x * view.scale,
            y: clientHeight / 2 - n.y * view.scale + 20,
          },
          true,
        )
      }
  ...
  const graphBody = (
    <div className="graph-frame">
          ...
          >
            {edges.map((edge, i) => {
              ...
              const color = kindColor(
                edge.kind === 'direct' ? 'route' : edge.kind === 'indirect' ? 'func' : 'module',
              )
              const dx = t.x - s.x
              const cx = s.x + dx * 0.5
              return (
                <g key={`e${i}`} opacity={active ? 1 : 0.1}>
                  <path
                    ...
                    strokeLinecap="round"
                    strokeDasharray={
                      edge.kind === 'imports' || edge.kind === 'indirect'
                        ? '5 5'
              ...
              return (
                ...
                >
                  ...
                  <circle
                    ...
                    className={
                      isSel ? 'graph-node-circle graph-node-circle-selected' : 'graph-node-bubble'
                    }
                  />
                  <text
                    x={0}
      ...
      <div className="graph-controls">
        <button
          title="Zoom out"
          onClick={() => setView((v) => ({ ...v, scale: clampZoom(v.scale * 0.85) }))}
        >
          <svg
            width="14"
            height="14"
        ...
        <button
          title="Zoom in"
          onClick={() => setView((v) => ({ ...v, scale: clampZoom(v.scale * 1.15) }))}
        >
          <svg
            width="14"
            height="14"
  ...
  if (isFullscreen) {
    return createPortal(
      <div className="graph-fullscreen">
        ...
        <div className="graph-viewer graph-fullscreen-body">{graphBody}</div>
      </div>,
      document.body,
    )
  }
